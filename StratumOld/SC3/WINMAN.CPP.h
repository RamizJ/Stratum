...
{
    if(!prj) prj=(TProject*)GetCurrentProject();
    if(prj)
    {
    ...
    if (_obj2d || fwKeys & HJ_LOADHYPER)
    {
        ...
        switch(key.openmode)
        {
            ...
            case 2://загружаем проект
                if(key.target)
                {
                    TProject* prevProject = ::project;
                    //DWORD _e=EXEC_FLAGS;
                    char dst[270];
                    char name[270];
                    BOOL unloadOnDeactivate = prevProject->unloadondeactivate;
                    if(unloadOnDeactivate)
                    {
                        if(prevProject->realname)
                            lstrcpy(name, prevProject->realname);
                        else
                            lstrcpy(name, prevProject->file);

                        meta->project = 0;
                    }
                    prevProject->GetFullFileName(key.target, dst, 1);
                    ...
                    if(LoadProject(dst, unloadOnDeactivate))
                    {
                        ...
                        //_prj->_exec_flags=_e;
                        if(!unloadOnDeactivate)
                            ::project->SetProjectProp("activateproject",prevProject->file);
    ...
    if (count && (((EXEC_FLAGS&EF_STEPACTIVE)==0)||(EXEC_FLAGS&EF_ONDIALG)) /*&& (state==0)*/)
    {
        ...
        switch(message)
        {
            ...
            case WM_CANCLOSE:
            {
                ...
                /************************************************************
         K E Y B O A R D   K E Y B O A R D   K E Y B O A R D
************************************************************/
            case WM_CHAR:
            case WM_KEYUP:
            case WM_KEYDOWN:
...
BOOL TMSGItems::RegisterObject(PObject po,HOBJ2D obj,INT16 code,UINT32 flags)
{
    ...
    switch(code){
        ...
        case WM_CONTROLNOTIFY:	s="_hObject:HANDLE,wNotifyCode";break;
        case WM_SPACEINIT:			s="_hSpace:HANDLE,_WindowName:STRING,_Target:STRING";break;
        case WM_SPACEDONE:
        case WM_DESTROY:				s=NULL;break;
        case WM_GETMINMAXINFO:	s="MaxSizeX,MaxPositionX,MinTrackSizeX,MaxTrackSizeX,MaxSizeY,MaxPositionY,MinTrackSizeY,MaxTrackSizeY";break;
        case WM_COMMAND:				s="idItem,wNotifyCode";break;
        case WM_SIZE:   				s="fwSizeType,nWidth,nHeight";break;
        case WM_MOVE:   				s="xPos,yPos";break;

        case WM_CANCLOSE:  		s="CanClose";break;
        case WM_ALLKEYMESSAGE:
        case WM_CHAR:
        case WM_KEYUP:
        case WM_KEYDOWN:				s="wVkey,Repeat,ScanCode";break;
...
void TWinItem::Send()
{
    if (obj_to_activate)
    {
        ...
        if(__target)lstrcpy(openmsg.target,__target);
                   ...
                   int x,int y,int sizex,int sizey,DWORD __flags,HWND _parent)
{
    ...
    DWORD win_style=0;
    mode=WIN_MDI;
    if (_class && filename)return;
    lstrcpyn(name,winname,MAX_WIN_NAME);

    if (_hsp)hsp=_hsp;
    else
    {
        if (_class){hsp=GetClassSpace(_class);}
        if (filename)
        {
    ...
    if (filename)_filename = NewStr(filename);
    BYTE showtype=0;
    if (hsp)
    {
        TSpaceData data(hsp);
        if (__flags&mWS_BYSPACE)
        {
            if (data.IsDefined())
            {
                if (data.style&SWF_NORESIZE){_flags|=mWS_NORESIZE;}
                if (data.sizex>0 && data.sizey>0)
                {
                    win_cx=data.sizex;
                    win_cy=data.sizey;
                }
                if(data.style&SWF_NORESIZE)nochangesize=1;
                if(data.style&SWF_MINIMIZE)showtype=2;
                if(data.style&SWF_MAXIMIZE)showtype=1;
                if(data.style&SWF_SPACESIZE)_flags|=mWS_SPACESIZE;
                if(data.style&SWF_AUTOORG)  _flags|=mWS_AUTOORG;
                if(data.style&SWF_DIALOG)   mode=WIN_DIALOG;//_flags|=mWS_DIALOG;
                ...
                if (data.style&SWF_MDI)      mode=WIN_MDI;   //_flags|=mWS_MDI;
                //if (data.style&SWF_CHILD)    mode=WIN_CHILD; //_flags|=mWS_MDI;
                if (data.style&SWF_MODAL)    _flags|=mWS_MODAL;
                if (data.style&SWF_VSCROLL)  _flags|=mWS_VSCROLL;
                if (data.style&SWF_HSCROLL)  _flags|=mWS_HSCROLL;
                if (data.style&SWF_AUTOSCROLL) _flags|=mWS_AUTOSCROLL;
            }
        }
        if (__flags&mWS_SPACESIZE) _flags|=mWS_SPACESIZE;
        if (__flags&mWS_AUTOORG)   _flags|=mWS_AUTOORG;
        if (__flags&mWS_MODAL)     _flags|=mWS_MODAL;
        if (__flags&mWS_VSCROLL)   _flags|=mWS_VSCROLL;
        ...
        POINT2D spaceorg;
        spaceorg.x=spaceorg.y=0;
        if (_flags&mWS_NORESIZE)nosize=1;
        ...
        if(_flags&mWS_BORDER)  win_style|=WS_BORDER;
#ifdef WIN32
...
void TMSGWindow::ProcessNewSpace(){
    ...
    while ((obj=GetObjectFromZOrder2d(hsp,i))!=0){
        ...
        switch(t){
            case otUSEROBJECT2D:if(GetFrameProp2d(hsp,obj,&fp)){
                    if(fp.flags&V2_AUTOPLAY){
                        VIDEOPROP vp;
                        GetVideoProp(fp.video,&vp);
                        UINT32 flags=0;
                        if(fp.flags&V2_CYCLED)flags|=PF_CYCLED;
...
{
    if(messages)messages->DoneHandler(NULL,WM_SPACEDONE);
    // Грохаем старое
    DeleteSpace2d(hsp);
    hsp=_hsp;
    if(messages)
...
HSP2D TWinItem::Replace(PClass _class,char*file,HSP2D _hsp){
    if (_class==NULL && (file==NULL|| *file==0))return hsp;
    if (_class){if (!lstrcmp(_class->name,_classname))return hsp;}
    if (file){if (!lstrcmp(file,_filename))return hsp;}

    if (!_hsp){
        if (_class){
            _hsp=GetClassSpace(_class);
        }else{_hsp=LdSpace2d(0,file);
            DeleteEditFrame2d(_hsp);
        }
    }
...
HSP2D TWindowManager::OpenSchemeWindow(char*wname,char* _classn,char* style)
{
    if(*wname=='@')
    {
        if(!lstrcmpi(wname,"@StratumFrame@")){return 0;}
#ifndef RUNTIME
        if(!lstrcmpi(wname,"@Watch@")){mainwindow->Cm_Watch();}else
            if(!lstrcmpi(wname,"@Text@")){mainwindow->CreateTextWindow(GetClassByName(_classn));}else
                if(!lstrcmpi(wname,"@Scheme@")){mainwindow->CreateSchemeWindow(_classn,NULL);}else
                    if(!lstrcmpi(wname,"@Image@")){mainwindow->CreateImageWindow(_classn);}else
                        if(!lstrcmpi(wname,"@Messages@")){mainwindow->Cm_Message();}else
                            if(!lstrcmpi(wname,"@Libraries@")){mainwindow->Cm_Libs();}else
                                if(!lstrcmpi(wname,"@Hierarchy@")){mainwindow->Cm_Hier();}
#endif
    }else
    {
        TWinItem*w=GetWindow(wname);
        ...
        if (w)
        {
            if(w->_classname){delete w->_classname;w->_classname=NULL;}
...
HSP2D TWindowManager::CreateWindowEx(char*wname,char *parent,char *data,HSP2D _hsp,int x,int y,int cx,int cy,char *style)
{
    if(lstrlen(wname)==0)	return 0;
    TWinItem*w=GetWindow(wname);
    TWinItem*wp=GetWindow(parent);
    //если передали имя родительского окна,
    //но такого окна не существует, то выходим
    if (lstrlen(parent) && (wp==0))	return 0;

#ifdef WIN32
    if(x== 32768) x=CW_USEDEFAULT;
    if(y== 32768) y=CW_USEDEFAULT;
    if(cx==32768) cx=CW_USEDEFAULT;
    if(cy==32768) cy=CW_USEDEFAULT;
#endif
    ...
    if(w)
        return w->Replace(_class,_file,_hsp);
    DWORD f=ProcessFlags(style);
    win_x=x;		win_y=y;
    win_cx=cx;	win_cy=cy;
    HWND _parent=0;
    ...
    {
        _parent=wp->HWindow;
        if(wp->mode==TWinItem::WIN_MDI)
            _parent=wp->mdichild->_client;
    }
    TWinItem*item=new TWinItem(wname,_hsp,_class,_file,win_x,win_y,win_cx,win_cy,f,_parent);
    if(item && wp && (item->mode==TWinItem::WIN_CHILD))
        item->Parent=wp;
    return _PostCreateWindow(InsertWin(item));
};
...
BOOL TWindowManager::RegisterObject(PObject po,char * winname,HOBJ2D obj,INT16 code,UINT32 flags){
    ...
    if (w){
        if(!w->window->messages)w->window->messages=new TMSGItems(w->window->HWindow,w->hsp);
        w->window->messages->RegisterObject(po,obj,code,flags);
    }
    return 0;
};
...
void TWindowManager::HideProjectWindows(){
    for(int i=0;i<count;i++){
        TWinItem*w=(TWinItem*)At(i);
        if(w->project)::ShowWindow(w->HWindow,SW_HIDE);
    }
};
...
INT16  TMetaFile::ProcessMetaRecord(INT16 code){
    if(proc){ return proc(meta,code,data);}
    return 0;
};
...
TMetaFile::~TMetaFile(){
    if(meta){meta->Truncate();delete meta;}
};
...
{if ((meta->status==0))
    {
        ...
        while(retry){
            ...
            {INT16 uc;
                ...
                if(_code==0 || (_code==uc)) ProcessMetaRecord(uc);else retry=TRUE;
                meta->Seek(pos);
                if(!retry) return uc;
            }
        };return 0;
